...
if os.path.exists(file_2025) and os.path.exists(file_2026):
    ...
    * 현재 팀원 배정(8명)은 필요 인원 대비 **{req_ppl_2026 - 8:.1f}명 부족**한 상태이며, 웹앱 자동화 같은 극한의 효율화 없이는 정상적인 업무 수행이 불가능한 수치입니다.
    """)

    col1, col2 = st.columns(2)
    with col1:
        fig_bar = go.Figure()
        fig_bar.add_trace(go.Bar(name='2025년', x=['팀리더', '팀원(실무)'], y=[leader_2025, staff_2025_sum], text=[f"{leader_2025:,.0f}h", f"{staff_2025_sum:,.0f}h"], textposition='auto', marker_color='#1f77b4'))
        fig_bar.add_trace(go.Bar(name='2026년', x=['팀리더', '팀원(실무)'], y=[leader_2026, staff_2026_sum], text=[f"{leader_2026:,.0f}h", f"{staff_2026_sum:,.0f}h"], textposition='auto', marker_color='#00A699'))
        fig_bar.update_layout(title="연간 과업 수행시간 비교 (팀리더 vs 팀원)", barmode='group', height=400)
    ...
    with col2:
        ...
        fig_line.add_hline(y=8, line_dash="dash", line_color="red", annotation_text="현재 팀원 정원 (8명)", annotation_position="bottom right")
    ...
    현재 마케팅팀은 **필요 인원(11.5명) 대비 실제 인원(8명)** 이라는 극도로 타이트한 환경 속에서 일하고 있습니다. 
