...
if os.path.exists(file_2025) and os.path.exists(file_2026):
    ...
    * 현재 팀원 배정(10명)은 필요 인원 대비 **{req_ppl_2026 - 10:.1f}명 부족**한 상태이며, 웹앱 자동화 같은 극한의 효율화 없이는 정상적인 업무 수행이 불가능한 수치입니다.
    ...
    # (수정됨) Conclusion & Action Plan에 마케팅 분석 활용 방안 추가
    st.info(f"""
    🚀 **[Conclusion & Action Plan]**
    * **현황:** 현재 마케팅팀은 타이트한 실무 인원(실제 10명 vs 필요 {req_ppl_2026:.1f}명)으로 운영 중입니다.
    * **극복 전략:** 방대한 분석/관리 업무를 데이터 분석 기반 파이썬 웹앱으로 자동화하여 극단적인 효율(Reduce)을 달성 중입니다.
    * **향후 집중 방향:** 시스템으로 방어한 여력을 **'통합 영업(업무/영업용)'**과 **'마케팅분석 고도화 & 시각화 결과 공유'** 등에 집중 투입하여 비즈니스 임팩트 극대화를 위해 노력중입니다.
    * **핵심 전략 과제:** 시각화된 대시보드를 통해 핵심 수요처의 월별 가스 사용량 패턴과 이상 징후(타 연료 전환 가능성 등)를 실시간으로 모니터링하고, 이탈하기 전에 선제적으로 대응하는 **'방어형 수익 수성'**에 분석 역량에 집중하고 있습니다.
    """)
